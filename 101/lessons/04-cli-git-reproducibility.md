...
> A ship's logbook. Every voyage is recorded entry-by-entry: date, cargo loaded, heading, weather conditions, officer on watch. Given the same starting port, the same cargo manifest, and the same chart, a different captain following the same log will reach the same destination. The logbook *is* the reproducibility guarantee.

Your Makefile is the logbook: it records the exact C standard (`-std=c99`), the exact POSIX level (`-D_POSIX_C_SOURCE=200809L`), and the exact list of source files (`LIB_SRCS`, `CLI_SRCS`). `git` records every change to that logbook as a permanent, labelled entry — the commit. CI is the second captain who sails from a blank chart using only the committed logbook, on a fresh Ubuntu VM, with no memory of your local machine. If the voyage succeeds there too, the logbook is correct.

The physics matters here: a GM calculation in `analysis.c` must give the same answer on your laptop as on the port authority's server. That's only possible if the build is deterministic — same inputs, same flags, same binary, every time. The Makefile + git + CI pipeline is the mechanism that enforces it.
...
<svg viewBox="0 0 620 210" role="img" xmlns="http://www.w3.org/2000/svg"
  ...
  <title>CLI, Makefile, git, and CI pipeline for CargoForge-C</title>
  <desc>A left-to-right pipeline: source files feed into make with compiler flags, producing the cargoforge binary, which is committed with git and then verified by GitHub Actions CI on a clean Ubuntu VM.</desc>

  <!-- Source files box -->
  <rect x="10" y="60" width="110" height="90" rx="6" fill="none" stroke="currentColor" stroke-width="1.5" stroke-opacity="0.55"/>
  <text x="65" y="80" text-anchor="middle" font-size="10.5" fill="currentColor" font-weight="600">src/</text>
  <text x="65" y="97" text-anchor="middle" font-size="9.5" fill="currentColor" fill-opacity="0.75">parser.c</text>
  <text x="65" y="111" text-anchor="middle" font-size="9.5" fill="currentColor" fill-opacity="0.75">analysis.c</text>
  <text x="65" y="125" text-anchor="middle" font-size="9.5" fill="currentColor" fill-opacity="0.75">main.c  cli.c</text>
  <text x="65" y="139" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.5">+ 7 more…</text>

  <!-- Arrow src → make -->
  <line x1="120" y1="105" x2="155" y2="105" stroke="currentColor" stroke-width="1.5" stroke-opacity="0.5" marker-end="url(#arr)"/>

  <!-- make box (teal highlight) -->
  <rect x="155" y="55" width="130" height="100" rx="6" fill="#12A594" fill-opacity="0.10" stroke="#12A594" stroke-width="1.8"/>
  <text x="220" y="78" text-anchor="middle" font-size="11" fill="#12A594" font-weight="700">Makefile</text>
  <text x="220" y="96" text-anchor="middle" font-size="9.5" fill="currentColor" fill-opacity="0.80">CC = gcc</text>
  <text x="220" y="111" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.75">-std=c99</text>
  <text x="220" y="125" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.75">-D_POSIX_C_SOURCE=…</text>
  <text x="220" y="140" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.75">-O3 -Wall -lm</text>

  <!-- Arrow make → binary -->
  <line x1="285" y1="105" x2="320" y2="105" stroke="currentColor" stroke-width="1.5" stroke-opacity="0.5" marker-end="url(#arr)"/>

  <!-- Binary box -->
  <rect x="320" y="70" width="100" height="70" rx="6" fill="none" stroke="currentColor" stroke-width="1.5" stroke-opacity="0.55"/>
  <text x="370" y="95" text-anchor="middle" font-size="11" fill="currentColor" font-weight="600">./cargoforge</text>
  <text x="370" y="113" text-anchor="middle" font-size="9.5" fill="currentColor" fill-opacity="0.70">optimize</text>
  <text x="370" y="127" text-anchor="middle" font-size="9.5" fill="currentColor" fill-opacity="0.70">validate  info</text>

  <!-- Arrow binary → git -->
  <line x1="420" y1="105" x2="455" y2="105" stroke="currentColor" stroke-width="1.5" stroke-opacity="0.5" marker-end="url(#arr)"/>

  <!-- git commit box -->
  <rect x="455" y="60" width="100" height="90" rx="6" fill="none" stroke="currentColor" stroke-width="1.5" stroke-opacity="0.55"/>
  <text x="505" y="82" text-anchor="middle" font-size="11" fill="currentColor" font-weight="600">git commit</text>
  <text x="505" y="99" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.75">fix: null cargo</text>
  <text x="505" y="112" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.75">pointer on</text>
  <text x="505" y="125" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.75">parse error</text>

  <!-- CI banner below the arrow row -->
  <rect x="155" y="175" width="400" height="28" rx="5" fill="#12A594" fill-opacity="0.08" stroke="#12A594" stroke-width="1.2"/>
  <text x="355" y="194" text-anchor="middle" font-size="10" fill="#12A594" font-weight="600">GitHub Actions CI — clean Ubuntu VM: make → ./cargoforge examples/…</text>

  <!-- Arrow from git box down to CI banner -->
  <line x1="505" y1="150" x2="505" y2="175" stroke="#12A594" stroke-width="1.4" stroke-opacity="0.6" marker-end="url(#arrteal)"/>

  <!-- Labels below boxes -->
  <text x="65" y="162" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.50">source</text>
  <text x="220" y="168" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.50">build recipe</text>
  <text x="370" y="152" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.50">deterministic binary</text>
  <text x="505" y="162" text-anchor="middle" font-size="9" fill="currentColor" fill-opacity="0.50">permanent snapshot</text>

  <!-- Arrow marker -->
  <defs>
    <marker id="arr" markerWidth="7" markerHeight="7" refX="5" refY="3.5" orient="auto">
      <path d="M0,0 L7,3.5 L0,7 Z" fill="currentColor" fill-opacity="0.45"/>
    </marker>
    <marker id="arrteal" markerWidth="7" markerHeight="7" refX="5" refY="3.5" orient="auto">
      <path d="M0,0 L7,3.5 L0,7 Z" fill="#12A594" fill-opacity="0.7"/>
    </marker>
  </defs>
</svg>
