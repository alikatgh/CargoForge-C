...
You'll forget the grammar — hold THIS picture instead:

> Imagine a hospital pharmacy that stores medications in labelled drawers. Each drawer has a number (1–9) on the front. A strict chart on the wall says "drawer 1 must never be opened in the same room as drawer 5." The pharmacist doesn't read every label from scratch — they check the number, look it up on the chart, and either allow it or refuse.

`parse_dg_field` is the intake clerk who reads the label on arrival and writes the number (`dg_class`) and product code (`un_number`) onto a small card (`DGInfo`). If the label is missing or illegible, the card is torn up (`cargo.dg = NULL`) and the item goes to the slower general-hazmat queue instead.

`imdg_get_segregation` is the wall chart — a 17 × 17 grid. Each `(dg_class, dg_division)` pair maps to a row/column index, and the cell value tells you the minimum metres between those two drawers on the ship. A value of `5` (`SEG_INCOMPATIBLE`) means the two items cannot share the vessel at all. `imdg_check_all` is the final round where someone walks every pair of labelled items and measures the actual distance against the chart.
...
<svg viewBox="0 0 640 340" role="img" xmlns="http://www.w3.org/2000/svg"
  ...
  <title>IMDG parse flow in CargoForge-C</title>
  <desc>Manifest line enters parse_dg_field, which fills a DGInfo struct (dg_class, dg_division, un_number, stowage, ems). dg_class drives an index lookup into the 17×17 segregation matrix, returning a SegregationType (SEG_NONE through SEG_INCOMPATIBLE). A missing or invalid field returns NULL and falls through to the legacy hazmat rule.</desc>

  <!-- Manifest line box -->
  <rect x="20" y="20" width="200" height="50" rx="6" fill="none" stroke="currentColor" stroke-width="1.5"/>
  <text x="120" y="40" text-anchor="middle" font-size="11" fill="currentColor" font-weight="600">Manifest line (5th field)</text>
  <text x="120" y="58" text-anchor="middle" font-size="10" fill="currentColor" opacity="0.7">DG:3.1:UN1203:A:F-E,S-D</text>

  <!-- Arrow down to parse_dg_field -->
  <line x1="120" y1="70" x2="120" y2="105" stroke="currentColor" stroke-width="1.5" marker-end="url(#arr)"/>

  <!-- parse_dg_field box -->
  <rect x="20" y="105" width="200" height="40" rx="6" fill="#12A594" fill-opacity="0.15" stroke="#12A594" stroke-width="1.5"/>
  <text x="120" y="120" text-anchor="middle" font-size="11" fill="#12A594" font-weight="600">parse_dg_field()</text>
  <text x="120" y="136" text-anchor="middle" font-size="10" fill="currentColor" opacity="0.7">parser.c · strtok_r, calloc</text>

  <!-- Arrow right to DGInfo struct -->
  <line x1="220" y1="125" x2="295" y2="125" stroke="currentColor" stroke-width="1.5" marker-end="url(#arr)"/>

  <!-- DGInfo struct box -->
  <rect x="295" y="75" width="165" height="110" rx="6" fill="none" stroke="currentColor" stroke-width="1.5"/>
  <text x="377" y="94" text-anchor="middle" font-size="11" fill="currentColor" font-weight="600">DGInfo</text>
  <line x1="295" y1="100" x2="460" y2="100" stroke="currentColor" stroke-width="1" opacity="0.4"/>
  <text x="305" y="115" font-size="10" fill="currentColor">dg_class    <tspan fill="#12A594">3</tspan></text>
  <text x="305" y="130" font-size="10" fill="currentColor">dg_division <tspan fill="#12A594">1</tspan></text>
  <text x="305" y="145" font-size="10" fill="currentColor">un_number   <tspan fill="#12A594">"UN1203"</tspan></text>
  <text x="305" y="160" font-size="10" fill="currentColor">stowage     <tspan fill="#12A594">STOW_ANY</tspan></text>
  <text x="305" y="175" font-size="10" fill="currentColor">ems         <tspan fill="#12A594">"F-E,S-D"</tspan></text>

  <!-- Arrow down from DGInfo to matrix lookup -->
  <line x1="377" y1="185" x2="377" y2="220" stroke="currentColor" stroke-width="1.5" marker-end="url(#arr)"/>

  <!-- class_to_index / imdg_get_segregation box -->
  <rect x="270" y="220" width="220" height="40" rx="6" fill="#12A594" fill-opacity="0.15" stroke="#12A594" stroke-width="1.5"/>
  <text x="380" y="236" text-anchor="middle" font-size="11" fill="#12A594" font-weight="600">imdg_get_segregation()</text>
  <text x="380" y="252" text-anchor="middle" font-size="10" fill="currentColor" opacity="0.7">class_to_index → seg_matrix[i][j]</text>

  <!-- Arrow right to result -->
  <line x1="490" y1="240" x2="545" y2="240" stroke="currentColor" stroke-width="1.5" marker-end="url(#arr)"/>

  <!-- Result box -->
  <rect x="545" y="215" width="80" height="50" rx="6" fill="none" stroke="currentColor" stroke-width="1.5"/>
  <text x="585" y="234" text-anchor="middle" font-size="10" fill="currentColor" font-weight="600">SEG_*</text>
  <text x="585" y="248" text-anchor="middle" font-size="9" fill="currentColor" opacity="0.7">0–4 = metres</text>
  <text x="585" y="260" text-anchor="middle" font-size="9" fill="#D05663">5 = reject</text>

  <!-- NULL path -->
  <line x1="120" y1="145" x2="120" y2="190" stroke="#D05663" stroke-width="1.5" stroke-dasharray="5,3" marker-end="url(#arrd)"/>
  <rect x="20" y="190" width="200" height="38" rx="6" fill="#D05663" fill-opacity="0.10" stroke="#D05663" stroke-width="1.5"/>
  <text x="120" y="207" text-anchor="middle" font-size="10" fill="#D05663" font-weight="600">Invalid / missing field → NULL</text>
  <text x="120" y="221" text-anchor="middle" font-size="9" fill="currentColor" opacity="0.7">cargo.dg = NULL · legacy 3 m rule</text>

  <!-- imdg_check_all label at bottom -->
  <rect x="270" y="278" width="220" height="32" rx="5" fill="none" stroke="currentColor" stroke-width="1" stroke-dasharray="4,3" opacity="0.5"/>
  <text x="380" y="293" text-anchor="middle" font-size="10" fill="currentColor" opacity="0.7">imdg_check_all — O(n²) post-placement</text>
  <text x="380" y="305" text-anchor="middle" font-size="9" fill="currentColor" opacity="0.55">audit after find_best_fit_3d completes</text>
